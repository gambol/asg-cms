...
public class ParserWorker {
			...
			if (parser.parseOk()) {
				...
				try {
					...
					if (rh.isThisUrlNew(key)) {
						...
						logger.info("[store]already has it. donot store. url:"
								+ parser.getUrl());
					}
				} catch (Exception e) {
					logger.error("Exception in store url:" + parser.getUrl() +  " msg:" + e.getMessage());
				} finally {
					rh.destory();
				}
			} else {
				logger.info("parser fail. url:" + parser.getUrl());
			}
		}
	...
	public static void main(String[] args) {
		YaolanZhishiParser yzp = new YaolanZhishiParser("",
				"http://www.yaolan.com/zhishi/baobaobanzhengyuliunian/", 1);
