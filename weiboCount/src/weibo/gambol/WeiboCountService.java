...
public class WeiboCountService {
	private static final Logger logger = Logger.getLogger(WeiboCountService.class); 
	
	private static int maxQueryUidNum = 3;  // 最多取多少个用户进行抽样他的粉丝数目
	private static int maxQueryRepostUidNum = 1; // 每次取转发了此条微博的200个用户,最多取多少次
	private static int everyQueryRepostCount = 200; // 每次取200个.. sina定的
	
	private AccessToken accessToken;
	private Timeline tm;
	
	public WeiboCountService(AccessToken at) {
		accessToken = at;
		tm = new Timeline();
		tm.setToken(accessToken.getAccessToken());
	}
	 ...
	 * @throws Exception
	 */
	public int getFollowerNumbyWeiboId(String weiboId) throws Exception{
	 ...
	 * @throws Exception
	 */
	public int getFollwerNumByWidSet(Set<String> widSet) throws Exception {
		...
		while(it.hasNext()) {
			...
			if (index % step == 0) {
				getNum++;				
				allFollowerNum += getFollowerNumbyWeiboId(wid);
			}
			index++;
		}
	 ...
	 * 请求接口1次
	 */
	public int getRepostNumbyWid(String weiboId) throws Exception{
	 ...
	 * @return
	 */
	public double  getAllUidRepostWid(String weiboId, int allRepostNum, Set<String> set) throws Exception{
		...
		for (int i = 0; i < queryTimes; i = (int) (i + step)) {
			...
			if (thisTotalNum < everyQueryRepostCount) {
				logger.debug("getTotalNum:" + thisTotalNum + " page count:" + page.getCount() + "page'spage:" + page.getPage());
				break;
			}
		}
	...
	public WeiboCountResult countWeibo(String wid) throws Exception{
		String weiboId = "";
		WeiboCountResult wcr = new WeiboCountResult();
		try {
			JSONObject jid = tm.QueryId(wid, 1,1);
...
//		Log.logInfo("weibo id:" + weiboId);
		...
		// 查看这个用户本身的粉丝
		int listerCount = getFollowerNumbyWeiboId(weiboId);
		
		// 查看这条微博被转发的人
		int forwardNum = getRepostNumbyWid(weiboId);
		wcr.setForwardNum(forwardNum);
//		Log.logInfo("forward num:" + forwardNum);
		
		TreeSet<String> widSet = new TreeSet<String>(); 
		double ampTimes = getAllUidRepostWid(weiboId, forwardNum, widSet);
